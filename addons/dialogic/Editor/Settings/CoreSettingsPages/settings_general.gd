...
func _close_active_timeline() -> Resource:
	...
	settings_editor.editors_manager.clear_editor(timeline_node, true)
...
func _on_resave_tool_pressed() -> void:
	var prev_timeline := _close_active_timeline()
	if tool_thread and tool_thread.is_alive():
		tool_thread.wait_to_finish()

	tool_thread = Thread.new()
	tool_progress_mutex = Mutex.new()
	tool_thread.start(resave_tool)

	_silently_open_timeline(prev_timeline)
...
func _process(delta: float) -> void:
	if (tool_thread and tool_thread.is_alive()) or %ToolProgress.value < 1:
		...
		%ToolProgress.visible = %ToolProgress.value != 1
...
func resave_tool() -> void:
	...
	for timeline_identifier in timelines:
		...
		tool_progress_mutex.lock()
		tool_progress = 1.0/len(timelines)*index
		tool_progress_mutex.unlock()
		index += 1
	tool_progress_mutex.lock()
	tool_progress = 1
